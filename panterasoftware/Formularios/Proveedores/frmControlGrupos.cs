...
namespace panterasoftware.Formularios.Proveedores
{
    public partial class frmControlGrupos : Form
    {
        ...
        {
            App_Code.Proveedores.Grupos oRegistro = new App_Code.Proveedores.Grupos(int.Parse(this.lblFicha.Text));
            ...
            try
            {
                if (!Validar())
                {
                    if (oRegistro.Err)
                    {
                        oFuncion.Leer(panterasoftware.Formularios.frmPrincipal.id_grupo_usuario, "0200202");
                        if (oFuncion.Err)
                        {
                    ...
                    {
                        oFuncion.Leer(panterasoftware.Formularios.frmPrincipal.id_grupo_usuario, "0200203");
                        if (oFuncion.Err)
                        {
        ...
        {
            App_Code.Proveedores.Grupos oRegistro = new App_Code.Proveedores.Grupos(id);
            ...
            this.txtCodigo.BackColor = Color.White;
            this.txtNombre.BackColor = Color.White;
            oFuncion.Leer(panterasoftware.Formularios.frmPrincipal.id_grupo_usuario, "0200204");
            if (!oFuncion.Err)
            {
        ...
        private void Borrar()
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        App_Code.Proveedores.Grupos oRegistro = new App_Code.Proveedores.Grupos(int.Parse(this.lblFicha.Text.ToString()));
                        oRegistro.Eliminar();
                        this.Nuevo();
                        MessageBox.Show(oRegistro.Msg, "Mensaje", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    }
        ...
        private void frmControlGrupos_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.F1:
        ...
        private void frmControlGrupos_FormClosed(object sender, FormClosedEventArgs e)
        {
